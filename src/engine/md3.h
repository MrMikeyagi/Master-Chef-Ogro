...
struct md3 : vertmodel
{
    ...
    bool loaddefaultparts()
    {
        ...
        if(tex==notexture) conoutf("%s %s", SV_MESG50, name1);
        return true;
    }
...
{   
    if(!loadingmd3) { conoutf("%s", SV_MESG51); return; }
...
{
    if(!loadingmd3 || loadingmd3->parts.empty()) { conoutf("%s", SV_MESG51); return; }
...
void md3anim(char *anim, int *frame, int *range, float *speed, int *priority)
{
    ...
    if(anims.empty()) anims.add(NUMANIMS); //conoutf("%s %s", SV_MESG52, anim);
    /*else*/ loopv(anims)
    {
...
void md3skinanim(char* meshname, char* tex, int *frame)
{    
    loopmd3skins(meshname, s,
...
{
    if(!loadingmd3) { conoutf("%s", SV_MESG51); return; }
    if(!loadingmd3->parts.inrange(*parent) || !loadingmd3->parts.inrange(*child)) { conoutf("%s", SV_MESG53); return; }
    if(!loadingmd3->parts[*parent]->link(loadingmd3->parts[*child], tagname)) conoutf("%s %s", SV_MESG54, loadingmd3->loadname);
}
...
COMMAND(md3skinanim, "ssi"); //meshname, tex, frame, range
