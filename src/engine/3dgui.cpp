...
VARP(guirollovercolour, 0, 0x00FFC0, 0xFFFFFF);
...
SVARP(guioverlaytex, "hud/2.2/guioverlay");
SVARP(guiskintex, "hud/2.2/guiskin");
SVARP(guislidertex, "hud/2.2/guislider");
SVARP(guiicondir, "hud/2.2/icons");
VARP(guierrors, 0, 0, 1);
...
struct gui : g3d_gui
{
    ...
    void slider(int &val, int vmin, int vmax, int color, char *label)
    {
        ...
        if(visible())
        {
            ...
            if(hit) color = guirollovercolour;
    ...
    void icon_(Texture *t, bool overlaid, bool tiled, int x, int y, int size, bool hit, int rotate = 0, int xoff = 0, int yoff = 0, Texture *glowtex = NULL, const vec &glowcolor = vec(1, 1, 1), Texture *layertex = NULL) 
    {
        ...
        vec color = hit ? vec((guirollovercolour / 65536) / 255.0f , ((guirollovercolour % 65536) / 255) / 255.0f, (guirollovercolour % 256) / 255.0f) : (overlaid ? vec(1, 1, 1) : light); 
        ...
        if(overlaid) 
        {
	    s_sprintfd(tname)("data/%s", guioverlaytex);
	    if(guierrors)
	    	overlaytex = textureload(tname, 3);
	    else
            	overlaytex = textureload(tname, 3, true, false);
    ...
    {		
        if(visible())
        {
	    s_sprintfd(tname)("data/%s", guislidertex);
 	    if(guierrors)
	    	slidertex = textureload(tname, 3);
	    else
            	slidertex = textureload(tname, 3, true, false);
            glEnable(GL_TEXTURE_2D);
    ...
    int button_(const char *text, int color, const char *icon, bool clickable, bool center)
    {
        ...
        if(visible())
        {
            ...
            if(hit && clickable) color = guirollovercolour;	
            int x = curx;	
            if(isvertical() && center) x += (xsize-w)/2;
        
            if(icon)
            {
                s_sprintfd(tname)("data/%s/%s", guiicondir, icon);
	    if(guierrors)
	    	icon_(textureload(tname, 3), false, false, x, cury, ICON_SIZE, clickable && hit);
	    else
            	icon_(textureload(tname, 3, true, false), false, false, x, cury, ICON_SIZE, clickable && hit);
                x += ICON_SIZE;
            }
    ...
    {
    	s_sprintfd(tname)("data/%s", guiskintex);
	if(guierrors)
		skintex = textureload(tname, 3, true, false);
	else
