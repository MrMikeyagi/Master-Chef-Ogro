...
SVAR(loadback, "1");
VAR(loadbacktype, 0, 1, 2);

extern string mname;

void loadbackground(int w, int h)
{
	if(loadbacktype==0) return; //in case loadback is off
	...
	if(loadbacktype == 2)
	{
		s_sprintf(lbtex)("packages/%s_big", mname);
		lb = textureload(lbtex, 0, true, false);
		if(lb==notexture)
		{
			s_sprintf(lbtex)("packages/%s", mname);
			lb = textureload(lbtex, 0, true, false);
		}
	}
	...
	if( loadbacktype == 1)
	{
		s_sprintf(lbtex)("loadback/%s", loadback);
		lb = textureload(lbtex, 0, true, false);
	}


	if(lb==notexture) return;
	else
	{
		glBindTexture(GL_TEXTURE_2D, lb->id);
...
void resetgl()
{
    ...
    s_sprintfd(lbtex)("loadback/%s", loadback);
    reloadtexture(lbtex);
    extern string mname;
    s_sprintf(lbtex)("packages/%s_big", mname);
    reloadtexture(lbtex);
    s_sprintfd(maptex)("packages/%s", mname);
    reloadtexture(maptex);
    if(!reloadtexture(*notexture) ||
       !reloadtexture(cl->loadimage()))
        fatal("failed to reload core texture");
    reloadfonts();
    inbetweenframes = true;
    computescreen(CALC7);
    extern void resetgamma();
	resetgamma();
    reloadshaders();
    reloadtextures();
    initlights();
...
void gl_drawframe(int w, int h)
{
    ...
    updatetexanims(); //__offtools__ update tex id's for texanimations
...
SVARP(guicursortex, "hud/2.3/guicursor");
...
void drawcrosshair(int w, int h)
{
    ...
    if(windowhit)
    {
        s_sprintfd(tname)("data/%s", guicursortex);
...
void gl_drawhud(int w, int h)
{
    if(!hidehud)
    {
        ...
        if(editmode)renderblendbrush();

        glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
        
        if (editmode) cursorupdate();
        
        extern int showenthelpers;
        extern void renderhelpers();
        if((showenthelpers==1 && editmode) || showenthelpers == 2) renderhelpers();
...
void computescreen(const char *text, Texture *t, const char *overlaytext)
{
	...
	loopi(2)
	{
...
#else
            int sz = min(512, max(256, (h - 192) / 2)), x = (w-sz)/2, y = min(384, h-256);
            glBegin(GL_QUADS);
            glTexCoord2f(0, 0); glVertex2f(x,    y);
        ...
        if(overlaytext)
        {
            int sz = min(512, max(256, (h - 192) / 2)), x = (w-sz)/2, y = min(384, h-256), tw = text_width(overlaytext); 
	    if(loadbacktype == 2)
	    {
	    	x = w-sz; y = h-(sz*2/3);
	    }
