...
static bool compileasmshader(GLenum type, GLuint &idx, const char *def, const char *tname, const char *name, bool msg = true, bool nativeonly = false)
{
    ...
    if(msg && err!=-1)
    {
        conoutf(CON_ERROR, "%s (%s:%s) - %s", SV_MESG77, tname, name, glGetString(GL_PROGRAM_ERROR_STRING_ARB));
    ...
    else if(msg && !native) conoutf(CON_ERROR, "%s:%s %s", tname, name, SV_MESG78);
...
static void showglslinfo(GLhandleARB obj, const char *tname, const char *name)
{
    ...
    if(length > 1)
    {
        ...
        conoutf(CON_ERROR, "%s (%s:%s)", SV_MESG79, tname, name);
        puts(log);
        delete[] log;
    }
}
...
void setshader(char *name)
{
    ...
    if(!s)
    {
        if(renderpath!=R_FIXEDFUNCTION) conoutf(CON_ERROR, "%s %s", SV_MESG80, name);
    }
    else curshader = s;
}
...
void setshaderparam(const char *name, int type, int n, float x, float y, float z, float w) 
{
    ...
    {
        conoutf(CON_ERROR, "%s..%d\n", SV_MESG81, MAXSHADERPARAMS-1);
        return;
    }
