...
VARFP(musicvol, 0, 60, 255, setmusicvol(musicvol));
...
void initsound()
{
    ...
    {
        conoutf(CON_ERROR, "%s %s", SV_MESG84, (size_t)Mix_GetError());
        return;
    }
...
const char *exts[] = { "", ".mp3", ".wav", ".mid", ".ogg" };

void music(char *name, char *cmd)
{
	if(nosound || !musicvol || !soundvol) return;
	stopsound();
	...
	if(*name)
	{
		...
		loopi(sizeof(paths)/sizeof(paths[0]))
		{
			...
			{
				s_sprintf(sn)("%s/%s%s", paths[i], name, exts[j]);
	...
	if (!mod) conoutf(CON_ERROR, "%s %s", SV_MESG86, name);
}
...
void playsound(int n, const vec *loc, extentity *ent)
{
    ...
    if(!sounds.inrange(n)) { conoutf(CON_WARN, "%s %d", SV_MESG87, n); return; }
    ...
    if(!slot.s->sound)
    {
        ...
        if(!slot.s->sound) { conoutf(CON_ERROR, "%s %s", SV_MESG88, buf); return; }
    }
...
ICOMMAND(entsoundname, "i", (int *id), 
	...
	{s_sprintfd(s)("%s", mapsounds[*id].s->name); result(s);}
);


SVAR(SV_MESG114, "");

void writemapsounds(FILE *f)
{
	loopv(mapsounds)
	{
		fprintf(f, "mapsound \"%s\" %d %d // %d\n", mapsounds[i].s->name, mapsounds[i].vol, mapsounds[i].maxuses, i);
	}
}
