...
struct obj : vertmodel
{
    ...
    bool loaddefaultparts()
    {
        ...
        if(tex==notexture) conoutf("%s %s", SV_MESG115, name1);
        return true;
    }
...
{
    if(!loadingobj) { conoutf(SV_MESG116); return; }
    ...
    if(!mdl.meshes) conoutf("%s %s", SV_MESG117, filename); // ignore failure
    else mdl.initskins();
}

void objpitch(float *pitchscale, float *pitchoffset, float *pitchmin, float *pitchmax)
{
    if(!loadingobj || loadingobj->parts.empty()) { conoutf(SV_MESG116); return; }
