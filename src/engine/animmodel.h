...
struct animmodel : model
{
    ...
    //__tha__ see custom animations 
    struct vecanimspec 
    {
        vector<animspec> spec;
    };
    ...
    struct skin
    {
        ...
        bool skinanim; //__offtools__ skin animations
    hashtable<int, GLuint> anim; //frame, tex id 
     
        skin() : owner(0), tex(notexture), masks(notexture), envmap(NULL), unlittex(NULL), normalmap(NULL), shader(NULL), spec(1.0f), ambient(0.3f), glow(3.0f), specglare(1), glowglare(1), fullbright(0), envmapmin(0), envmapmax(0), translucency(0.5f), scrollu(0), scrollv(0), alphatest(0.9f), alphablend(true), cullface(true), skinanim(false) {}
        ...
        void bind(mesh *b, const animstate *as)
        {
            ...
            if(s!=lasttex || skinanim) //__offtools__ skin animation)
            {
                if(enableglow) glActiveTexture_(GL_TEXTURE1_ARB);
                if (anim.access(as->cur.fr1))
		{
			glBindTexture(GL_TEXTURE_2D, anim[as->cur.fr1]); //__offtools__ change texture depending on frame
		 }
		else 
		{
    ...
    struct part
    {
        ...
        vector<vecanimspec> *anims; //vector for custom animation instead fixed array
        int numanimparts;
        ...
        part() : meshes(NULL), numanimparts(1), pitchscale(1), pitchoffset(0), pitchmin(0), pitchmax(0), translate(0, 0, 0)
        {
            ...
            anims = new vector<vecanimspec>[MAXANIMPARTS];
        }
        virtual ~part()
        {
             loopi(MAXANIMPARTS) 
        ...
        bool calcanim(int animpart, int anim, int basetime, int basetime2, dynent *d, int interp, animinfo &info)
        {
            ...
            else 
            {
                animspec *spec = NULL;
                //if(anims[animpart])
                if(anims[animpart].length() > 0)
                {
                    vector<animspec> &primary = anims[animpart][anim&ANIM_INDEX].spec;
                    if(primary.length()) spec = &primary[uint(varseed + basetime)%primary.length()];
                    if((anim>>ANIM_SECONDARY)&(ANIM_INDEX|ANIM_DIR))
                    {
                        vector<animspec> &secondary = anims[animpart][(anim>>ANIM_SECONDARY)&ANIM_INDEX].spec;
                        if(secondary.length())
                        {
        ...
        void setanim(int animpart, int num, int frame, int range, float speed, int priority = 0)
        {
            ...
            //add/reserve vector elements to for standard animations 
		if (anims[animpart].length() == 0) 
		{
			for (int i = 0; i < NUMANIMS; i++) { anims[animpart].add(); }
		}
		else if (num == NUMANIMS) //__tha__ add new element for custom animation  
		{
			anims[animpart].add();
			num = anims[animpart].length()-1;
		}
		//add new animspec
		animspec &spec = anims[animpart][num].spec.add();
            spec.frame = frame;
            spec.range = range;
            spec.speed = speed;
    ...
    void render(int anim, int basetime, int basetime2, const vec &o, float yaw, float pitch, dynent *d, modelattach *a, const vec &color, const vec &dir)
    {
        if(!loaded) return;
