...
static SDL_Surface *texturedata(const char *tname, Slot::Tex *tex = NULL, bool msg = true, bool *compress = NULL)
{
    ...
    if(!tname)
    {
        ...
        if(tex->name[0]=='<') 
        {
            ...
            if(!file) { if(msg) conoutf(CON_ERROR, "%s packages/%s", SV_MESG90, tex->name); return NULL; }
            file++;
        }
        else file = tex->name;
    ...
    else if(tname[0]=='<') 
    {
        cmds = tname;
        file = strrchr(tname, '>');
        if(!file) { if(msg) conoutf(CON_ERROR, "%s %s", SV_MESG90, file); return NULL; }
        file++;
    }
    ...
    if(!s) { if(msg) conoutf(CON_ERROR, "%s %s", SV_MESG90, file); return NULL; }
    int bpp = s->format->BitsPerPixel;
    if(!texformat(bpp)) { SDL_FreeSurface(s); conoutf(CON_ERROR, "%s %s", SV_MESG91, file); return NULL; }
...
void texture(char *type, char *name, int *rot, int *xoffset, int *yoffset, float *scale)
{
    ...
    s.loaded = false;
    s.texmask |= 1<<tnum;
    if(s.sts.length()>=8) conoutf(CON_WARN, "%s %d", SV_MESG92, curtexnum);
...
void autograss(char *name)
{
    ...
    if(strstr(name, "packages"))
    	s.autograss = newstring(name);
    else 
    	s.autograss = newstring(name[0] ? makerelpath("packages", name) : "packages/textures/grass_kurt2");
}
...
Texture *cubemaploadwildcard(Texture *t, const char *name, bool mipit, bool msg)
{
    ...
    loopi(6)
    {
        ...
        {
            if(surface[i] && msg) conoutf(CON_ERROR, "%s : %s", SV_MESG93, sname);
            loopj(i) SDL_FreeSurface(surface[j]);
            return NULL;
        }
...
Texture *cubemapload(const char *name, bool mipit, bool msg)
{
    ...
    if(!strchr(pname, '*'))
    {
        s_sprintfd(jpgname)("%s_*", pname);
        t = cubemaploadwildcard(NULL, jpgname, mipit, false);
        if(!t && msg) conoutf(CON_ERROR, "%s %s", SV_MESG94, name);
    }
...
//ToDo: change gluScaleImage2d with quicker scaler
//      test depth 
/*void overwritetexture(int slot, uchar* buf, int w, int h, int format, int type, bool scale = false)
{
  Slot &dst = lookuptexture(slot, false);
  //ToDo: give me some more options :)
  if (!dst.sts[0].t || format != GL_RGB || type != GL_UNSIGNED_BYTE) { return; }
  glBindTexture(GL_TEXTURE_2D, dst.sts[0].t->id);

  if ( dst.sts[0].t->w == w && dst.sts[0].t->h == h )
  {
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, dst.sts[0].t->w, dst.sts[0].t->h , GL_RGB, GL_UNSIGNED_BYTE, buf);
    return;
  }
  else if (!scale && w < dst.sts[0].t->w && h < dst.sts[0].t->h)
  {
    int xoff = (dst.sts[0].t->w -w) / 2;
    int yoff = (dst.sts[0].t->h -h) / 2;
    glTexSubImage2D(GL_TEXTURE_2D, 0, xoff, yoff, w, h , GL_RGB, GL_UNSIGNED_BYTE, buf);
    return;
  }
  else
  {
  ...
  if (scale)
  {
    uchar scaled[dst.sts[0].t->w * dst.sts[0].t->h * 3];
    gluScaleImage(GL_RGB, w, h , GL_UNSIGNED_BYTE, buf, dst.sts[0].t->w, dst.sts[0].t->h, GL_UNSIGNED_BYTE, scaled);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, dst.sts[0].t->w, dst.sts[0].t->h , GL_RGB, GL_UNSIGNED_BYTE, scaled);      
  }
}*/ //defunct, needs to be rewritten/modified to not use glu

hashtable<int, TexAnim> texanim;

void updatetexanims()
{
  enumerate(texanim, TexAnim, anim, 
              if (!anim.frames.length() || !anim.on) { return; }
              if (anim.delay && lastmillis-anim.lastupdate >= anim.delay)
              {
                anim.lastupdate = lastmillis;
                anim.slot->sts[0].t->id = anim.frames[anim.pos];
                if (anim.pos < anim.frames.length()-1) { anim.pos++; }
                else { anim.pos = 0; }
              }
            );
}
...
{
  Slot &slot = lookuptexture(*n, true);
  if (!slot.sts[0].t) { return; }
  if(!texanim.access(*n))
  {
    TexAnim &anim = texanim[*n];
    anim.slot = &slot;
    anim.frames.add(slot.sts[0].t->id);
  }
}
...
void texanimadd(int *d, int *s)
{
  if(texanim.access(*d))
  {
    Slot &dst = lookuptexture(*d, true);
    Slot &src = lookuptexture(*s, true);
    if (src.sts[0].t && dst.sts[0].t)
    {
      texanim[*d].frames.add(src.sts[0].t->id);
    }
  }
}
...
{
  if(texanim.access(*n)) { texanim[*n].on = true; }     
}

void texanimdelay(int *n, int *d)
{
  if(texanim.access(*n) && *d) { texanim[*n].delay = *d; }
}

void texanimstop(int *n)
{
  if(texanim.access(*n)) { texanim[*n].on = false; }     
}
...
void texanimreset(int *n)
{
  ...
  { 
    TexAnim& anim = texanim[*n]; 
    anim.on = false; 
    anim.slot->sts[0].t->id = anim.frames[0];
  }     
}
