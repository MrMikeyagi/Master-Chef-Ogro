...
{
    if(!editmode) { conoutf(CON_ERROR, "%s", SV_MESG67); return true; }
    ...
    if(!viewable) conoutf(CON_ERROR, "%s", SV_MESG68);
    return !viewable;
}
...
void cursorupdate()
{
    ...
    if(havesel)
    {
		...
		(a=sel.o.tovec()).x=0; (b=sel.s.tovec()).x=hdr.worldsize/sel.grid; boxs3D(a, b, sel.grid);  
		(a=sel.o.tovec()).y=0; (b=sel.s.tovec()).y=hdr.worldsize/sel.grid; boxs3D(a, b, sel.grid);  
		(a=sel.o.tovec()).z=0; (b=sel.s.tovec()).z=hdr.worldsize/sel.grid; boxs3D(a, b, sel.grid);
        }
    }
...
void swapundo(undolist &a, undolist &b, const char *s)
{
    ...
    if(a.empty()) { conoutf(CON_WARN, "%s %s", SV_MESG69, s); return; }	
...
{
    if(noedit()) return;
    if(reptex < 0) { conoutf(CON_ERROR, "%s", SV_MESG70); return; }
...
void editmat(char *name)
{
    ...
    if(id<0) { conoutf(CON_ERROR, "%s \"%s\"", SV_MESG71, name); return; }
...
{ 
    if(!editmode) { conoutf(CON_ERROR, "%s", SV_MESG67); return; }
