...
{
    if(overrideidents) { conoutf(CON_ERROR, "%s %s", SV_MESG28, code); return; }
    keym &km = keyms[*code];
    km.code = *code;
    DELETEA(km.name);
...
{
    if(overrideidents) { conoutf(CON_ERROR, "%s %s \"%s\"", SV_MESG29, cmd, key); return; }
    keym *km = findbind(key);
    if(!km) { conoutf(CON_ERROR, "$%s \"%s\"", SV_MESG30, key); return; }
