...
void loadsky(const char *basename, Texture *texs[6])
{
	...
	s_sprintfd(name)("%s_%s", makerelpath("packages", basename), side);
        if((texs[i] = textureload(name, 3, true, false))==notexture) conoutf(CON_ERROR, "%s %s", SV_MESG1, name);
    }
}
...
{
    s_sprintfd(name)("packages/%s", basename);
