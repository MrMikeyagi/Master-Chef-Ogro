...
VARW(flarecutoff, 0, 1000, 10000);
VARW(flaresize, 20, 100, 500);
...
struct flarerenderer : partrenderer
{
    ...
    void addflare(vec &o, uchar r, uchar g, uchar b, bool sun, bool sparkle, int sizemod)
    {
        ...
        {
            mod = ((flarecutoff * (sizemod > 0 ? (sizemod / 100.0f) : 1))-vec(o).sub(center).squaredlen())/flarecutoff;
...
static flarerenderer flares("data/hirato/lensflares", 64);
