...
static void show_genpvs_progress(int unique = pvs.length(), int processed = numviewcells)
{
    ...
    s_sprintfd(text1)("%.2f%% - %d of %d view cells", float(bar1 * 100), processed, totalviewcells);
...
static void lockpvs_(bool lock)
{
    ...
    conoutf("%s %.1f, %.1f, %.1f", SV_MESG112, camera1->o.x, camera1->o.y, camera1->o.z);
}
...
void testpvs(int *vcsize)
{
    ...
    loopi(MAXWATERPVS) lockedwaterplanes[i] = waterplanes[i].height;
    lockpvs = 1;
    conoutf("%s %d: %.1f, %.1f, %.1f (%d B)",SV_MESG113, size, camera1->o.x, camera1->o.y, camera1->o.z, len);
...
void genpvs(int *viewcellsize)
{
    ...
    {
        conoutf(CON_ERROR, SV_MESG110);
        return;
    }

	extern string version, mname;
	s_sprintfd(capt)("Sandbox Engine %s: %s - Generating PVS Data...", version, mname);
