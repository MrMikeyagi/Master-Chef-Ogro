...
void show_calclight_progress()
{
    ...
    s_sprintfd(text1)("%.2f%%", float(bar1 * 100));
    s_sprintfd(text2)("%d textures %.2f%% utilized", lightmaps.length(), float(bar2 * 100));
...
void calclight(int *quality)
{
    ...
    {
        conoutf(CON_ERROR, "%s", SV_MESG35); 
        return;
    }
	extern string version, mname;
	s_sprintfd(capt)("Sandbox Engine %s: %s - Calculating Lightmaps...", version, mname);
...
void patchlight(int *quality)
{
    ...
    if(!setlightmapquality(*quality))
    {
        conoutf(CON_ERROR, "%s", SV_MESG37); 
        return;
    }
	extern string version, mname;
	s_sprintfd(capt)("Sandbox Engine %s: %s - Patching Lightmaps...", version, mname);
	SDL_WM_SetCaption(capt, NULL);
    computescreen(CALC3);
    cleanuplightmaps();
    progress = 0;
    ...
    check_calclight_progress = false;
    SDL_TimerID timer = SDL_AddTimer(250, calclight_timer, NULL);
    if(patchnormals) show_out_of_renderloop_progress(0, CALC4);
    Uint32 start = SDL_GetTicks();
