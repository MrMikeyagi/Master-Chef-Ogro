...
struct cubeloader
{
    ...
    void load_cube_world(char *mname)
    {
        ...
        if(!f) { conoutf(CON_ERROR, "%s %s", SV_MESG32, cgzname); return; }
        c_header hdr;
        ...
        { 
            ...
            {
                conoutf(CON_ERROR, "map %s %s", cgzname, SV_MESG33); 
                gzclose(f); 
                return; 
            }
        }
        else if(hdr.version>5) mod = true;
        if(hdr.version>5 && !mod) { conoutf(CON_ERROR, "map %s %s", cgzname, SV_MESG34); gzclose(f); return; }
        emptymap(12, true);
        freeocta(worldroot);
