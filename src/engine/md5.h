        ...
        bool loadmd5mesh(const char *filename, bool smooth)
        {
            ...
            for(;;)
            {
                ...
                else if(strstr(buf, "mesh {"))
                {
                    ...
                    {
                        conoutf("%s %s", SV_MESG55, filename);
                        meshes.removeobj(m);
                        delete m;
                    }
                }
...
{
    if(!loadingmd5) { conoutf("%s", SV_MESG56); return; }
    ...
    if(!mdl.meshes) conoutf("%s %s", SV_MESG57, filename); // ignore failure
    else 
    {
        mdl.initanimparts();
        mdl.initskins();
...
{
    if(!loadingmd5 || loadingmd5->parts.empty()) { conoutf("%s", SV_MESG56); return; }
    ...
    conoutf("%s %s %s %s", SV_MESG58_1,  name, SV_MESG58_2, tagname);
}
...
void md5pitch(char *name, float *pitchscale, float *pitchoffset, float *pitchmin, float *pitchmax)
{
    ...
    if(name[0])
    {
        ...
        conoutf("%s %s %s", SV_MESG59_1, name, SV_MESG59_2);
        return;
    }
...
void md5skinanim(char* meshname, char* tex, int *frame)
{    
    loopmd3skins(meshname, s,
        Texture *t = textureload(makerelpath(md3dir, tex), 0, true, false);
        s.anim[*frame] = t->id;
        s.skinanim = true;
    );
}
...
void md5anim(char *anim, char *animfile, float *speed, int *priority)
{
    ...
    if(anims.empty()) anims.add(NUMANIMS); //conoutf("%s %s", SV_MESG60, anim);
    //else
    //{
        ...
        if(!sa) conoutf("%s %s", SV_MESG61, filename);
        else loopv(anims)
        {
...
void md5animpart(char *maskstr)
{
    ...
    loopv(bonestrs)
    {
        ...
        if(bone<0) { conoutf("%s %s %s [%s]", SV_MESG62_1, bonestr, SV_MESG62_2, maskstr); bonestrs.deletecontentsa(); return; }
    ...
    if(!p->addanimpart(bonemask.getbuf())) conoutf(SV_MESG63);
}
...
{
    if(!loadingmd5) { conoutf("%s", SV_MESG56); return; }
    if(!loadingmd5->parts.inrange(*parent) || !loadingmd5->parts.inrange(*child)) { conoutf("%s", SV_MESG64); return; }
    if(!loadingmd5->parts[*parent]->link(loadingmd5->parts[*child], tagname)) conoutf("%s %s", SV_MESG65, loadingmd5->loadname);
}
...
COMMAND(md5skinanim, "ssi"); //meshname, tex, frame, range
