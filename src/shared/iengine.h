...
//__tha__ added for actors and scripts
extern void newent(char *what, int *a1, int *a2, int *a3, int *a4, int *a5);
...
extern void regularshape(int type, int radius, int color, int dir, int num, int fade, const vec &p, float size = 4.0f, float vel = 200.0f);
...
{
    DECAL_EMPTY = 0,
    DECAL_BURN, //activate when colision detection for particles are improved
    DECAL_STAIN,
    DECAL_RIPPLE
};
...
// client
enum { ST_EMPTY, ST_LOCAL, ST_TCPIP, ST_SCRIPTED };
...
extern bool shadowmapping, reflecting, envmapping;
